...
func (ess *ElasticsearchStore) Save(vectorRecord VectorRecord) (VectorRecord, error) {
	...
	if res.IsError() {
		return VectorRecord{}, errors.New("Error response from Elasticsearch: " + res.String())
	} else {
		return vectorRecord, nil
	}
}

func (ess *ElasticsearchStore) SearchTopNSimilarities(embeddingFromQuestion VectorRecord, size int) ([]VectorRecord, error) {
	...
	for _, hit := range r["hits"].(map[string]interface{})["hits"].([]interface{}) {
		...
		similarities = append(similarities, VectorRecord{
			Prompt: prompt,
			Id: docID.(string),
			Score: score,
		})
